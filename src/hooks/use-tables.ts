...
async function fetchTables(eventId: string) {
  const res = await fetch(`/api/celebrations/${eventId}/tables`);
...
export function useUpdateTable(eventId: string) {
  ...
  return useMutation({
    mutationFn: async ({ tableId, input }: { tableId: string; input: UpdateTableInput }) => {
      const res = await fetch(`/api/celebrations/${eventId}/tables/${tableId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(input),
      });
...
export function useDeleteTable(eventId: string) {
  ...
  return useMutation({
    mutationFn: async (tableId: string) => {
      const res = await fetch(`/api/celebrations/${eventId}/tables/${tableId}`, { method: "DELETE" });
